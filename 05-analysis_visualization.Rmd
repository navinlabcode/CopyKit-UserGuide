...
<!-- make sure to test this since it is not something that change be evaluated in the report. -->
...
<!-- I think this example should include using 'label' for color. -->
...
<!-- Something is writing incorrectly in the reference section here. -->
...
<!-- I was trying to avoid starting a sentance with the lowercase function name but I don't think it worked. -->
...
<!-- it might be good to add a little more description of what how to read the plots. -->
The boxplot shows the variation for each tested k with the red dots represent the mean.

<!-- This is very confusing because the red dot is described as different things -->
...
<!-- I think I might be having issue, but in the example, I don't see any gray. I just see blue to yellow gradient. -->
Clones not detected in a given k are presented in gray.
```{r plot_suggested_k_tile}
plotSuggestedK(tumor, geom = 'tile')
```
<!-- Choose an example that has undetected clones to show this -->
...
<!-- is there a less ugly example of this? -->
...
<!-- Make sure to double check this statement still worked it is not being evaluated in the run. -->
...
If the method used for clustering was *hdbscan*, it is possible that a subgroup of outliers is identified. Outliers are added to subgroup `c0` and may be removed by subsetting the CopyKit object. 
...
<!-- This needs a sentance or two about what an integer profile is and why you would want one (i.e.interpretability) -->
...
<!-- NOTE you have set this to not evaluate. Need to double check that it works outside of this. -->
...
<!-- I changed this from S to L because it looks like you used L in the actually graphs. --> 
...
Therefore we will set the accordingly to a difference of plus or minus 0.2 (leaving some room for error), compared to a segment ratio of 1.
...
<!-- This is throwing warnings, maybe get it to not do that in the guide? -->
